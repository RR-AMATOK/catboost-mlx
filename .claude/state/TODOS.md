...
- **#130** [pending] **S38-LG-SMALL-N-RESIDUAL**: small-N LG+Cosine drift not closed by DEC-042. T3 re-run on post-fix master: G3b N=1k drift 27-31%, G3c N=2k drift 43-45%. S33 Commit 3b LG-guard removal was validated only at N=50k (0.382%). Math-first investigation needed to determine why per-side mask is regime-dependent (partition-state at small leaf counts? priority-queue × small-N interaction? second independent mechanism?). Until closed, users running LG+Cosine at N < 10k may see significant drift vs CPU CatBoost — document in catboost/mlx/README.md as Known Limitation.
