...
- **#130** [in progress — H3 FIXED, H1 open] **S38-LG-SMALL-N-RESIDUAL**: FBSPP per-side mask (H3, S38-S0) ported 2026-04-25 — removes FBSPP joint-skip contribution (expected G3b drop from 27-31% to ~14%). H1 residual (~14% at N=1k) under parallel investigation in S38 PROBE-G. README Known Limitations updated. Gate numbers pending bash build + run. Until H1 closed, LG+Cosine at N < 10k still drifts vs CPU CatBoost — documented in catboost/mlx/README.md.
