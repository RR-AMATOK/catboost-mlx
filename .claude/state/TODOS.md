...
- [x] **#114 S31-T-CLEANUP** — COMPLETED 2026-04-25 (S37). SA-I2-S29 #95 CLI try/catch wrap shipped at `fb775a6765`; SA-L1-S33 path redaction at `9959161fd7`; SA-L3-S30 + SA-N2-S33 env-var hardening at `10a8734ebf`.
- [x] **#113 S31-T3-MEASURE** — RAN 2026-04-25 (S37). G3a PASS (aggregate drift 53.30% → 1.27% at N=50k). G3b/G3c FAIL post-fix (N=1k 27-31%, N=2k 43-45% — basically unchanged from pre-fix). DEC-042 fix is regime-dependent. Verdict: `docs/sprint37/t3-rerun/verdict.md`. Spawns #130 S38-LG-SMALL-N-RESIDUAL.
- [x] **#130 S38-LG-SMALL-N-RESIDUAL** — RESOLVED-CONFIG-ARTIFACT 2026-04-25. PROBE-Q phase 2 located the root cause: harness configuration asymmetry (CPU `random_strength=0.0`, MLX default `RandomStrength=1.0`). With matched RS=0, MLX trees are bit-identical to CPU's at the F2 N=1k anchor (12/12 splits match feat+border, train RMSE 0.000% drift). 5-seed verification at RS=0 parity: 0.000% drift all seeds; at RS=1.0 parity: mean -3.43% ± 1.06% (real but bounded RNG-implementation difference, MLX slightly favorable). The "13–44% small-N drift" Sprint 37 #113 T3 G3b/G3c flagged was the gap between MLX-with-noise and CPU-deterministic. DEC-042 stays correct. Harness fix (`--random-strength 0` in F2 + PROBE-G scripts) shipped. README updated. See DEC-045, `docs/sprint38/probe-q/PHASE-2-FINDING.md`. H3 (FBSPP per-side mask port, commit `a481972529`) was correct but its drift contribution was masked by the harness asymmetry; the port is structurally beneficial regardless. H1 was a phantom.
