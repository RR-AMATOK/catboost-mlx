...
**Status**: PARTIAL — Adult + Higgs-1M + Higgs-11M landed; Epsilon / Amazon / MSLR pending data acquisition
...
### Finding 1 — MLX is bit-equivalent to CatBoost-CPU at fair convergence
...
| **Higgs-1M** | **1000** | 1M × 28 | 0 (0%) | **+0.0002** | **fp32 numerical noise — ~bit-equivalent** |

**At fair convergence (Higgs-1M, 1000 iters), MLX agrees with CatBoost-CPU to within 0.0002 logloss.** That is within fp32 numerical noise — essentially the floor a different reduction order can reach. The S43-T2 1000-iter rerun showed that the +0.0012 "architectural floor" reported in DEC-046 was itself partly a methodology artifact of under-convergence at 200 iterations. The DEC-046 numeric-only-bounded-gap claim now strengthens to a **bit-equivalence claim** at fair convergence.

The categorical-encoding asymmetry on Adult (61% of the +0.1695 gap) remains unaddressed — Adult overfits at 1000 iters across all 4 frameworks, so iter-count tuning doesn't help here. Closing the cat asymmetry is the optional Lane D investigation per DEC-046.
