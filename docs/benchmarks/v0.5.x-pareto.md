...
**Status**: PARTIAL — Adult + Higgs (1M subset) landed; Epsilon / Amazon / MSLR pending data acquisition
...
## TL;DR (current state: Adult + Higgs-1M)

Two patterns emerge across the two datasets we have so far. **The MLX-vs-CPU gap depends almost entirely on whether the dataset has categorical features:**
...
**The "if you can pre-encode categoricals, the MLX gap is bounded" claim from DEC-046 is now validated on a second independent dataset.** Higgs has zero categoricals; MLX agrees with CatBoost-CPU to within 0.0012 logloss (about 0.2%). On Adult, the categorical-encoding share is 61% of the gap (DEC-046 mechanism class).

XGBoost (CPU hist) is **on the Pareto frontier on both datasets** — fastest train AND best metric, by a clear margin. CatBoost-MLX is the slowest framework on both at our 200-iteration setting.
...
## Higgs (1M-row subset — full 11M run pending)
