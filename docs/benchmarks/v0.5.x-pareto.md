...
### Finding 1 — MLX accuracy is bounded; gap depends on categoricals (DEC-046 mechanism)
...
| **Higgs-1M** | 1M × 28 | 0 (0%) | **+0.0012** | architectural floor only |
| **Higgs-11M** | 10.5M × 28 | 0 (0%) | **+0.0013** | architectural floor only — *bound holds at 10× scale* |

The DEC-046 "numeric-only ⇒ bounded gap" claim now holds across **30,000× variation in dataset size** (32k Adult numeric to 10.5M Higgs full). The architectural-floor magnitude on numeric-only is consistently ~0.001 logloss; this is essentially the fp32 reduction-order disagreement floor (mathematician estimate: 2e-4 theoretical, ~6× above floor in practice).
