"""CatBoost-CUDA runner for the cross-class benchmark.

Identical to the upstream CPU runner, but trains with task_type='GPU'.
Used to measure CatBoost-CUDA wall-clock and metric on the same datasets +
hyperparameters that the M3 Max MLX run measured, so the two are directly
comparable.

Output JSONs are written to --results-dir and follow the same schema as the
M3 Max sweep, so they can be aggregated and plotted by the existing
benchmarks/upstream/scripts/aggregate_results.py.

Run: python -m scripts.run_catboost_cuda --dataset adult --seed 42
"""
...
def _coerce_cats_to_int(X: np.ndarray, cat_indices) -> np.ndarray:
    ...
    pandas DataFrame where cat cols are int64; numeric cols stay float."""
    if not cat_indices:
        return X
    import pandas as pd
    df = pd.DataFrame(X, columns=[f"f{i}" for i in range(X.shape[1])])
    for j in cat_indices:
        df[f"f{j}"] = df[f"f{j}"].fillna(-1).astype(np.int64)
    return df
...
def _train_ranker(X_tr, y_tr, g_tr, X_te, y_te, g_te, seed):
    ...
    X_tr_s, y_tr_s, g_tr_s = X_tr[order_tr], y_tr[order_tr], g_tr[order_tr]
    X_te_s, y_te_s, g_te_s = X_te[order_te], y_te[order_te], g_te[order_te]
...
def main() -> int:
    ...
    print(f"[catboost_cuda/{args.dataset}/seed={args.seed}] "
    ...
    if task == "classification":
        ...
        y_proba, train_s, pred_s, _ = _train_classifier(
