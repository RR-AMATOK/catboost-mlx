...
try:
    import resource  # POSIX-only (macOS/Linux); not available on Windows
    _HAS_RESOURCE = True
except ImportError:
...
def peak_rss_bytes() -> int:
    ...
    Windows: `resource` module unavailable; fall back to psutil if installed,
             else return 0 (RSS is metadata-only, not load-bearing for the
             benchmark numbers).
    """
    if _HAS_RESOURCE:
